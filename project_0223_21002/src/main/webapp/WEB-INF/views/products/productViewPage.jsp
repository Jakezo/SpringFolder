						...
						<tbody>
							<tr class=" xans-record-">
								<c:forEach var="productDto" items="${prodViewlist}">
									<c:if
										test="${productDto.p_Size eq 'S' or productDto.p_Size eq 'FREE'}">
										<td colspan="2"><span
											style="font-size: 13px; color: #000000;">
												${productDto.p_Name} [${productDto.p_Color}] </span></td>
									</c:if>
								</c:forEach>
							</tr>
							<tr class=" xans-record-">
								<td colspan="2"><span
									style="font-size: 12px; color: #555555;"><br />
									<br />
										<h6>FABRIC</h6> <c:forEach var="productDto"
											items="${prodViewlist}">
											<c:if
												test="${productDto.p_Size eq 'S' or productDto.p_Size eq 'FREE'}">
												${productDto.p_Fabric}
									...
									<br />
										<h6>INFORMATION</h6> <c:forEach var="productDto"
											items="${prodViewlist}">
											<c:if
												test="${productDto.p_Size eq 'S' or productDto.p_Size eq 'FREE'}">
							...
							<tr class=" xans-record-">
								<th scope="row"><span
									style="font-size: 12px; color: #000000;">price</span></th>
								<td><span style="font-size: 12px; color: #000000;">
										<strong id="span_product_price_text"> <c:forEach
												var="productDto" items="${prodViewlist}">
												<c:if
													test="${productDto.p_Size eq 'S' or productDto.p_Size eq 'FREE'}">
													₩<fmt:formatNumber value="${productDto.p_Price}"
														pattern="###,###,###" />
												</c:if>
											</c:forEach>
							...
							<tr class=" xans-record-">
								<th scope="row"><span
									style="font-size: 12px; color: #555555;">적립금</span></th>
								<td><span style="font-size: 12px; color: #555555;">
										<img
										src="//img.echosting.cafe24.com/design/skin/admin/ko_KR/ico_pay_money.gif"
										alt="무통장 입금 결제" style="margin-bottom: 2px;"> <img
										src="//img.echosting.cafe24.com/design/skin/admin/ko_KR/ico_pay_card.gif"
										alt="신용카드 결제" style="margin-bottom: 2px;"> <c:forEach
											var="productDto" items="${prodViewlist}">
											<c:if
												test="${productDto.p_Size eq 'S' or productDto.p_Size eq 'FREE'}">
												<fmt:formatNumber value="${productDto.p_Price * 0.01}"
													pattern="###,###,###" />원 (1%)
											</c:if>
										</c:forEach> <input id="product_price" name="product_price" value=""
										type="hidden">
								</span></td>
							</tr>
							<tr class=" xans-record-">
							...
							<tr id="inav">
								<td>SIZE</td>
								<td><select id="std_Size" name="std_Size">
										<option value="">[필수] 옵션을 선택해 주세요</option>
										<c:forEach var="productDto" items="${prodViewlist}">
											<c:set var="ea" value="EA" />
											<c:if test="${productDto.p_Amount ne 0}">
												<option value="${productDto.p_No}">${productDto.p_Size}
													남은재고:${productDto.p_Amount}</option>
											</c:if>
											<c:if test="${productDto.p_Amount eq 0}">
												<option value="out-of-stock">${productDto.p_Size}
													--품절</option>
											</c:if>
										</c:forEach>
								</select></td>
							...
							<tr id="inav">
								...
								<td><input type="number" name="o_amount" id="o_amount"
									min="1" max="1" value="1" /></td>
							</tr>
							<tr class="btntr">
								<td colspan="2"><input type="hidden" name="m_no" id="m_no"
									value="${loginDto.m_no}" /> <input type="hidden" name="p_No"
									id="p_No" value="" /><br /> <input type="button" id="btndo"
									class="addCart_btn" value="카트에 담기" onclick="fn_cart(this.form)" />
									<input type="button" id="btndo" value="구매하기"
									onclick="fn_order(this.form)" /></td>
							</tr>
						</tbody>
					</table>
				</div>
									...
									<c:forEach var="productDto" items="${prodViewlist}">
										<c:if test="${reviewDto.rv_p_no eq productDto.p_No}">
											...
											<tr style="display: none;">
												<td colspan="4">
													<div id="imgs">
														<c:set var="uploadFileNames"
															value="${fn:split(reviewDto.rv_filename,'^')}" />
														<p>${reviewDto.rv_content}</p>
														<c:forEach var="uploadFileName" items="${uploadFileNames}">
															<img style="width: 300px;" alt="${uploadFileName}"
																src="resources/storage/${uploadFileName}">
															<br />
															<br />
														</c:forEach>
													</div>
									...
									<c:if test="${qnaDto.q_p_no eq productDto.p_No}">
										...
										<tr style="display: none;">
											<td colspan="4">
												<div id="imgs">
													<c:set var="uploadFileNames"
														value="${fn:split(qnaDto.q_filename,'^')}" />
